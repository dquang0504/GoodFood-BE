...
type ProductType struct {
	...
	TypeName      string `boil:"typeName" json:"typeName" toml:"typeName" yaml:"typeName"`
...
var ProductTypeWhere = struct {
	...
	ProductTypeID: whereHelperint{field: "\"product_type\".\"productTypeID\""},
	TypeName:      whereHelperstring{field: "\"product_type\".\"typeName\""},
	Status:        whereHelperbool{field: "\"product_type\".\"status\""},
}

// ProductTypeRels is where relationship names are stored.
var ProductTypeRels = struct {
	ProductTypeIDProducts string
}{
	ProductTypeIDProducts: "ProductTypeIDProducts",
}

// productTypeR is where relationships are stored.
type productTypeR struct {
	ProductTypeIDProducts ProductSlice `boil:"ProductTypeIDProducts" json:"ProductTypeIDProducts" toml:"ProductTypeIDProducts" yaml:"ProductTypeIDProducts"`
}
...
func (o *ProductType) GetProductTypeIDProducts() ProductSlice {
	if o == nil {
		return nil
	}

	return o.R.GetProductTypeIDProducts()
}

func (r *productTypeR) GetProductTypeIDProducts() ProductSlice {
	if r == nil {
		return nil
	}

	return r.ProductTypeIDProducts
}
...
var (
	productTypeAllColumns            = []string{"productTypeID", "typeName", "status"}
	productTypeColumnsWithoutDefault = []string{"typeName", "status"}
	productTypeColumnsWithDefault    = []string{"productTypeID"}
	productTypePrimaryKeyColumns     = []string{"productTypeID"}
	productTypeGeneratedColumns      = []string{"productTypeID"}
)
...
// ProductTypeIDProducts retrieves all the product's Products with an executor via productTypeID column.
func (o *ProductType) ProductTypeIDProducts(mods ...qm.QueryMod) productQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"product\".\"productTypeID\"=?", o.ProductTypeID),
	)

	return Products(queryMods...)
}

// LoadProductTypeIDProducts allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (productTypeL) LoadProductTypeIDProducts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProductType interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`product`),
		qm.WhereIn(`product.productTypeID in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.ProductTypeIDProducts = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ProductTypeID == foreign.ProductTypeID {
				local.R.ProductTypeIDProducts = append(local.R.ProductTypeIDProducts, foreign)
				if foreign.R == nil {
					foreign.R = &productR{}
				}
...
// AddProductTypeIDProducts adds the given related objects to the existing relationships
// of the product_type, optionally inserting them as new records.
// Appends related to o.R.ProductTypeIDProducts.
// Sets related.R.ProductTypeIDProductType appropriately.
func (o *ProductType) AddProductTypeIDProducts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Product) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.ProductTypeID = o.ProductTypeID
			...
			values := []interface{}{o.ProductTypeID, rel.ProductID}
	...
	if o.R == nil {
		o.R = &productTypeR{
			ProductTypeIDProducts: related,
		}
	} else {
		o.R.ProductTypeIDProducts = append(o.R.ProductTypeIDProducts, related...)
	}
...
func ProductTypes(mods ...qm.QueryMod) productTypeQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"product_type\".*"})
	}
...
func FindProductType(ctx context.Context, exec boil.ContextExecutor, productTypeID int, selectCols ...string) (*ProductType, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"product_type\" where \"productTypeID\"=$1", sel,
	)

	q := queries.Raw(query, productTypeID)
...
func (o *ProductType) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		wl = strmangle.SetComplement(wl, productTypeGeneratedColumns)
...
func (o *ProductType) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, productTypeGeneratedColumns)
		update = strmangle.SetComplement(update, productTypeGeneratedColumns)

		if updateOnConflict && len(update) == 0 {
			return errors.New("models: unable to upsert product_type, could not build update column list")
		}

		ret := strmangle.SetComplement(productTypeAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(productTypePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert product_type, could not build conflict column list")
			}

			conflict = make([]string, len(productTypePrimaryKeyColumns))
			copy(conflict, productTypePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"product_type\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *ProductType) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"product_type\" WHERE \"productTypeID\"=$1"
...
func (o *ProductType) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindProductType(ctx, exec, o.ProductTypeID)
	if err != nil {
		return err
	}
...
func ProductTypeExists(ctx context.Context, exec boil.ContextExecutor, productTypeID int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"product_type\" where \"productTypeID\"=$1 limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, productTypeID)
	}
	row := exec.QueryRowContext(ctx, sql, productTypeID)
...
func (o *ProductType) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return ProductTypeExists(ctx, exec, o.ProductTypeID)
}
