...
func testTransactionsExists(t *testing.T) {
	...
	e, err := TransactionExists(ctx, tx, o.TransactionID)
	if err != nil {
...
func testTransactionsFind(t *testing.T) {
	...
	transactionFound, err := FindTransaction(ctx, tx, o.TransactionID)
	if err != nil {
		t.Error(err)
	}
...
func testTransactionsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(transactionPrimaryKeyColumns, transactionColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testTransactionToOneAccountUsingAccountIDAccount(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadAccountIDAccount(ctx, tx, false, (*[]*Transaction)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTransactionToOneInvoiceUsingInvoiceIDInvoice(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TransactionSlice{&local}
	if err = local.L.LoadInvoiceIDInvoice(ctx, tx, false, (*[]*Transaction)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTransactionToOneSetOpAccountUsingAccountIDAccount(t *testing.T) {
	var err error
	...
	for i, x := range []*Account{&b, &c} {
		...
		if x.R.AccountIDTransactions[0] != &a {
...
func testTransactionToOneSetOpInvoiceUsingInvoiceIDInvoice(t *testing.T) {
	var err error
	...
	for i, x := range []*Invoice{&b, &c} {
		...
		if x.R.InvoiceIDTransactions[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
...
var (
	transactionDBTypes = map[string]string{`TransactionID`: `integer`, `TransactionCode`: `character varying`, `CardNumber`: `character varying`, `FullName`: `character varying`, `TransactionDate`: `date`, `Status`: `boolean`, `Amount`: `real`, `AccountID`: `integer`, `InvoiceID`: `integer`}
	_                  = bytes.MinRead
)
...
func testTransactionsSliceUpdateAll(t *testing.T) {
	...
	if strmangle.StringSliceMatch(transactionAllColumns, transactionPrimaryKeyColumns) {
		...
		fields = strmangle.SetComplement(fields, transactionGeneratedColumns)
	}
