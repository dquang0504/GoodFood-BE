...
func testProductsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(productPrimaryKeyColumns, productColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testProductToManyProductIDCartDetails(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.ProductIDCartDetails().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadProductIDCartDetails(ctx, tx, false, (*[]*Product)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProductIDCartDetails); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ProductIDCartDetails = nil
	if err = a.L.LoadProductIDCartDetails(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProductIDCartDetails); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProductToManyProductIDInvoiceDetails(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.ProductIDInvoiceDetails().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := ProductSlice{&a}
	if err = a.L.LoadProductIDInvoiceDetails(ctx, tx, false, (*[]*Product)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProductIDInvoiceDetails); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ProductIDInvoiceDetails = nil
	if err = a.L.LoadProductIDInvoiceDetails(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProductIDInvoiceDetails); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProductToManyProductIDProductImages(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.ProductIDProductImages().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := ProductSlice{&a}
	if err = a.L.LoadProductIDProductImages(ctx, tx, false, (*[]*Product)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProductIDProductImages); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ProductIDProductImages = nil
	if err = a.L.LoadProductIDProductImages(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProductIDProductImages); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProductToManyProductIDReviews(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.ProductIDReviews().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := ProductSlice{&a}
	if err = a.L.LoadProductIDReviews(ctx, tx, false, (*[]*Product)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProductIDReviews); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ProductIDReviews = nil
	if err = a.L.LoadProductIDReviews(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ProductIDReviews); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProductToManyAddOpProductIDCartDetails(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddProductIDCartDetails(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ProductIDCartDetails[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ProductIDCartDetails[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ProductIDCartDetails().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testProductToManyAddOpProductIDInvoiceDetails(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddProductIDInvoiceDetails(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ProductIDInvoiceDetails[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ProductIDInvoiceDetails[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ProductIDInvoiceDetails().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testProductToManyAddOpProductIDProductImages(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddProductIDProductImages(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ProductIDProductImages[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ProductIDProductImages[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ProductIDProductImages().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testProductToManyAddOpProductIDReviews(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddProductIDReviews(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ProductIDReviews[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ProductIDReviews[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ProductIDReviews().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testProductToOneProductTypeUsingProductTypeIDProductType(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	local.ProductTypeID = foreign.ProductTypeID
	...
	if check.ProductTypeID != foreign.ProductTypeID {
		t.Errorf("want: %v, got %v", foreign.ProductTypeID, check.ProductTypeID)
	}
	...
	if err = local.L.LoadProductTypeIDProductType(ctx, tx, false, (*[]*Product)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testProductToOneSetOpProductTypeUsingProductTypeIDProductType(t *testing.T) {
	var err error
	...
	for i, x := range []*ProductType{&b, &c} {
		...
		if x.R.ProductTypeIDProducts[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.ProductTypeID != x.ProductTypeID {
		...
		if a.ProductTypeID != x.ProductTypeID {
			t.Error("foreign key was wrong value", a.ProductTypeID, x.ProductTypeID)
		}
	}
}
...
var (
	productDBTypes = map[string]string{`ProductID`: `integer`, `ProductName`: `character varying`, `Price`: `real`, `CoverImage`: `character varying`, `Description`: `character varying`, `Status`: `boolean`, `InsertDate`: `date`, `ProductTypeID`: `integer`, `Weight`: `real`}
	_              = bytes.MinRead
)
...
func testProductsSliceUpdateAll(t *testing.T) {
	...
	if strmangle.StringSliceMatch(productAllColumns, productPrimaryKeyColumns) {
		...
		fields = strmangle.SetComplement(fields, productGeneratedColumns)
	}
